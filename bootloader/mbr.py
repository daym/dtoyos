...
data = [
	...
	0xb9, 0x00, 0x01, # mov $256, %cx # 256 2-Byte blocks.
	...
	0x66, 0x8b, 0x46, 0x08, # mov 8(%bp), %eax
	0x66, 0xa3, 0x67 - 2 + 8, 0x06, # mov %eax, (...)
	#0x8b, 0x47, 0x0a, # mov 10(%bp), %ax
	#0xa3, 0x64 - 2 + 10, 0x06, # mov %ax, (...)

	0x66, 0x8b, 0x4e, 0x0c, # mov 12(%bp), %cx (hopefully regsize is OK)
	# if ecx is larger than 255, use 255.
	0x66, 0xb8, 0xff, 0x00, 0x00, 0x00, # mov $255, %eax
	0x66, 0x39, 0xc1, # cmp %eax, %ecx
	0x66, 0x0f, 0x4f, 0xc8, # cmovg $eax, %ecx
	0x89, 0x0e, 0x67 - 2 + 2, 0x06, # mov %cx, (0x065e)
	...
	0xbe, 0x67 - 2, 0x06, # mov $DAP_header -> ds:%si
...
assert(data.index(0x9b) == 0x67)
while len(data) < 0x1B8:
	data.append(0)
assert(len(data) == 0x1B8)
data += [
	0xAF, 0xFE, 0x10, 0x13, # disk id: 0x1310feaf
	0x00, 0x00,
	# partition table
	##0x40, 0x50, # magic
	##0x00, 0x00, # reserved
