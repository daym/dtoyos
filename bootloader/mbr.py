...
data = [
	...
	0xbb, 0xbe, 0x07, # mov $0x07be, %bx

	# partl1:
	0xe3, 0x0b, # jecxz parte1
	0x80, 0x3f, 0x80, # cmpb $0x80, %ds:(%di)
	0x74, 0x08, # je partok
	0x83, 0xc3, 0x10, # add $0x10, %bx
	0x49, # dec %cx
	...
	0x83, 0xc3, 0x08, # add $8, %bx
	0x8b, 0x07, # mov (%bx), %ax
	0x90, 0x90, 0x90, # 0xa3, 0x62, 0x06, # mov %ax, (0x0662)
	0x8b, 0x47, 0x02, # mov 2(%bx), %ax
	0x90, 0x90, 0x90, # 0xa3, 0x64, 0x06, # mov %ax, (0x0664)

	0x66, 0x8b, 0x4f, 0x04, # mov 4(%di), %cx (hopefully size is OK)
	0x89, 0x0e, 0x5c, 0x06, # mov %cx, (0x065c)
	...
	0xbe, 0x5c - 2, 0x06, # mov $DAP_header -> ds:%si
	0xb4, 0x42, # mov 0x42 -> %ah
	0xcd, 0x13, # int $0x13 # note: uses %ah, %dl, %ds:%si.
	0x72, 0x05, # jc hang

	# jmp to the loaded sectors.
	...
	0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, # starting sector
]
#print("LEN", len(data))
#print("9b", data.index(0x9b))
assert(data.index(0x9b) == 0x5c)
